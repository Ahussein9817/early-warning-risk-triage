...
- **Attempted 2026-08-29, found insufficiently reliable, abandoned per pre-set stopping criterion: LLM-applied rubric as a scaled ground-truth substitute (`src/llm_rubric_label.py`).** Goal was to scale past the 106-row hand-labeled sample by having an LLM (Claude Sonnet, via `claude -p`) apply the same hand-labeling rubric to the full 1,120-row test split. Validated first against the 106 known rows, blind: 62.3% agreement, heavily one-directional (37/40 disagreements were the LLM under-calling escalation). One refinement round (3 worked examples from real disagreement cases, explicit "lean toward escalated=1 under uncertainty" instruction): 60.2% agreement on the 103 rows not used as examples (3 excluded, no longer blind), no improvement, error direction unchanged (35/41 still under-calling). Stopping criterion (75-80% agreement, meaningfully less lopsided direction) was not met, so no Step 2 (scaling to the full test split) was attempted, and no further prompt iteration was attempted, per instruction given in advance rather than decided after seeing a disappointing number. `llm_rubric_label` is a distinct, separately-stored column/file (`data/labeled/llm_rubric_labels_validation.csv`), never merged into `hand_labeled.csv` or the Section 6 comparison report. See README Results for the full writeup; the hand-labeled sample size (n=106) remains an open limitation for future work, not something this resolved.
  - **Follow-up blind recheck:** the 35 disagreement rows were re-reviewed fresh (shuffled order, no labels visible, neither original nor LLM) to test whether the disagreement reflected human error rather than LLM conservatism. 33/35 (94.3%) reconfirmed as escalated; only 2/35 flipped, both no-narrative complaints outside the rubric's core categories. Confirms the hand-labeled ground truth is reliable and the LLM's conservative bias, not human labeling error, drives the 60.2% agreement rate.
