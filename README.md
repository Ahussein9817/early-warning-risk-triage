...
### Large-scale complement (proxy label, n=1,120)

The hand-labeled comparison above is small because hand-labeling doesn't scale. A complementary check reuses the model's own test split (1,120 rows, ~20x the hand-labeled sample, `src/large_scale_comparison.py`) scored against the proxy label instead. Neither measurement is unbiased alone: hand-labeled is closer to genuine severity judgment but tiny; the proxy-labeled test split is huge but inherits the proxy's own bias (see base-rate divergence above). One thing works in the proxy label's favor here that didn't hold for the hand-labeled sample: it was deliberately built independent of `has_narrative` (Section 2), so this comparison isn't confounded by narrative presence the way the hand-labeled one turned out to be.

| Subgroup | n | Baseline AUC | Fused AUC | Delta | Delta 95% CI | Fused wins in resamples |
|---|---|---|---|---|---|---|
| overall | 1,120 | 0.7878 | 0.7900 | +0.0021 | [-0.0025, +0.0065] | 83.3% |
| has_narrative | 487 | 0.7849 | 0.7901 | +0.0052 | [-0.0041, +0.0146] | 86.1% |
| no_narrative | 633 | 0.7895 | 0.7898 | +0.0003 | [-0.0015, +0.0020] | 61.9% |

At this scale, the CIs are narrow enough to be informative even though most still touch zero. `no_narrative`'s CI sits almost exactly on zero (61.9% fused wins, essentially a coin flip), which is the expected, mechanistically sound result: for rows with no narrative, fused's extra input is a constant zero vector, so it has almost nothing to differentiate it from baseline. `has_narrative` and `overall` lean consistently toward fused (86.1% and 83.3% of resamples respectively) with tight intervals mostly on the positive side, a smaller effect than the hand-labeled point estimate suggested, but far more precisely measured, and directionally consistent with it.

**Triangulated verdict:** both measurements, differently biased, point the same direction on `has_narrative`, fused very likely does not hurt, and plausibly helps a small amount, though "plausibly" is doing real work in that sentence: this evidence is against the proxy label, which is itself a biased measure of genuine escalation, so it shows fused predicts the proxy slightly better, not confirmed proof it captures true severity better. The hand-labeled result remains the more construct-valid one; it's just underpowered. Where they agree (direction) is meaningful; where the hand-labeled CI is wide open is a reminder not to overstate the magnitude.
...
Deployed via Streamlit Community Cloud, connected directly to this GitHub repo (main file path `dashboard/app.py`), rather than a separately-maintained deployment repo. Originally planned for HuggingFace Spaces; revised after HF deprecated free-tier Streamlit hosting in favor of a paid Docker-SDK-only model, see `OPEN_DECISIONS.md`. `dashboard/app.py` doesn't import from `src/`, its small amount of feature-encoding logic is duplicated locally rather than importing the full pipeline package, keeping the deployed app's dependency surface limited to what it actually runs.

## Results

**Verdict: the hand-labeled sample alone is inconclusive; triangulated with the large-scale proxy-label complement, the evidence leans toward a small, real but unconfirmed benefit from fusion, not the clean positive result the original point estimate suggested.** On the hand-labeled sample (n=106, the more construct-valid measurement), the `has_narrative` point estimate (+0.0174 AUC) has a 95% CI of **[-0.12, +0.16]**, crossing zero: alone, this cannot distinguish "fused helps" from "no real difference." Scored against the proxy label on the model's own 1,120-row test split instead (~20x more data, differently biased, see Evaluation → Large-scale complement), the same `has_narrative` comparison shows a smaller but far more precisely measured edge (+0.0052 AUC, fused winning 86.1% of bootstrap resamples). Both measurements point the same direction; neither alone settles it. `overall` and `no_narrative` on the hand-labeled sample show fused *underperforming* with real statistical support (95% CI entirely below zero), traced to the sub_issue-concession-rate mechanism, but that pattern does not replicate on the large-scale proxy-labeled test split (`no_narrative` there sits almost exactly at zero, the mechanistically expected result since fused's input is a constant zero vector for those rows). That disagreement is itself informative: it suggests the hand-labeled sample's `overall`/`no_narrative` finding may be an artifact of its small size and its own sub_issue-driven bias, not a robust structural weakness of the fused model.
...
**LLM-based label scaling was attempted and found unreliable, not used anywhere in Results above.** To address the sample-size limitation directly, an LLM (Claude Sonnet, via `claude -p`, `src/llm_rubric_label.py`) was given the same hand-labeling rubric (README Methodology, "Hand-labeling rubric") and applied it blind to the same 106 rows already hand-labeled, to validate before scaling to the full 1,120-row test split. Initial agreement: 62.3% (106/106 parsed cleanly), with a heavily one-directional error pattern, 37 of 40 disagreements were the LLM under-calling escalation relative to the human labeler, only 3 were over-calling. One round of refinement was tried: 3 worked examples pulled from the actual disagreement cases, plus an explicit instruction to resolve ambiguous cases toward escalated=1 (matching how the human labeler actually worked). Result on the 103 rows not used as examples (the 3 example rows excluded from this stat since they're no longer blind): **60.2% agreement, no improvement, and the error direction stayed just as lopsided** (35 of 41 disagreements still under-calling). Even one worked example's own near-duplicate case was still mislabeled 0 by the model. Per the pre-set stopping criterion (75-80% agreement with a meaningfully less lopsided direction), no further prompt iteration was attempted, this is reported as a finding, not resolved: the LLM's threshold for "specific, substantiated, serious" is more conservative than the human labeler's in a way a few calibration examples didn't shift. `data/labeled/llm_rubric_labels_validation.csv` and `llm_rubric_validation_report.json` are kept in the repo as the documented record of this attempt. **The hand-labeled sample remains n=106; this was not a workaround for that limitation, and should not be read as one.**

**Blind recheck of the 35 disagreement rows: the original hand labels mostly held up.** To test whether the disagreements reflected human labeling error rather than LLM conservatism, the 35 rows where the human said escalated and the LLM said not were re-reviewed fresh, order shuffled, no labels visible (neither the original hand label nor the LLM's), so the reassessment couldn't be anchored on the first pass. Result: **33 of 35 (94.3%) were reconfirmed as escalated**, matching the original hand label; only 2 of 35 flipped to match the LLM. The two that flipped were both no-narrative complaints in categories outside the core rubric focus (abusive language, dispute-notice timing), not the identity-theft/debt-not-owed/legal-threat categories the rubric examples were built around. This strengthens the conclusion that **the hand-labeled ground truth is reliable, and the LLM's conservative bias, not human labeling error, is the primary driver of the 60.2% LLM-vs-human agreement rate** found above.
